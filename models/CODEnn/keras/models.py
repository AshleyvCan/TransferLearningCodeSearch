...
class JointEmbeddingModel:
    ...
    def build(self):
        ...
        embedding = Embedding(input_dim=self.data_params['n_words_name'],
        ...
        if init_emb_weights is not None: init_emb_weights = [init_emb_weights]
        embedding = Embedding(input_dim=self.data_params['n_words_code'],
                              output_dim=self.model_params.get('n_embed_dims', 100),
                              weights=init_emb_weights,
        ...
        merged_code_repr=Concatenate(name='merge_coderepr')([methname_repr,tokens_repr])
        code_repr=Dense(self.model_params.get('n_hidden',400),activation='tanh',name='dense_coderepr')(merged_code_repr)
        
        
        self._code_repr_model=Model(inputs=[methname,tokens],outputs=[code_repr],name='code_repr_model')  #apiseq   
        ...
        if init_emb_weights is not None: init_emb_weights = [init_emb_weights]
        embedding = Embedding(input_dim=self.data_params['n_words_qt'],
                              output_dim=self.model_params.get('n_embed_dims', 100),
                              weights=init_emb_weights,
                              mask_zero=True,#Whether 0 in the input is a special "padding" value that should be masked out. 
                                      #If True, all subsequent layers must support masking, otherwise an exception will be raised.
        ...
        code_repr=self._code_repr_model([methname,tokens]) #apiseq
        desc_repr=self._desc_repr_model([desc])
        cos_sim=Dot(axes=1, normalize=True, name='cos_sim')([code_repr, desc_repr])
        
        sim_model = Model(inputs=[methname,tokens,desc], outputs=[cos_sim],name='sim_model') #apiseq
        ...
        '''
        good_sim = sim_model([self.methname,self.tokens, self.desc_good])# similarity of good output #self.apiseq,
        bad_sim = sim_model([self.methname,self.tokens, self.desc_bad])#similarity of bad output #self.apiseq,
        ...
        self._training_model=Model(inputs=[self.methname,self.tokens,self.desc_good,self.desc_bad], #self.apiseq,
