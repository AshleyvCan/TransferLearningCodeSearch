...
class SearchEngine:
    ...
    def train(self, model):
        ...
        for i in range(self.train_params['reload']+1, nb_epoch):
            ...
            hist = model.fit([methnames, tokens, good_descs, bad_descs], epochs=1, batch_size=batch_size, validation_split=split) #removed API
            ...
            if valid_every is not None and i % valid_every == 0:                
                accs,mrrs,maps,ndcgs, sr1s, sr5s, sr10s = self.valid(model, 1000, 1)             

    ##### Evaluation in the develop set #####
    
    
    def valid(self, model, poolsize, K, start_pool = 0):

        print(poolsize, start_pool)
        """
        ...
        if self._eval_sets is None:
            methnames = data_loader.load_hdf5(self.data_path+self.data_params['valid_methname'], start_pool, poolsize)
            #apiseqs= data_loader.load_hdf5(self.data_path+self.data_params['valid_apiseq'], 0, poolsize)
            tokens = data_loader.load_hdf5(self.data_path+self.data_params['valid_tokens'], start_pool, poolsize)
            descs = data_loader.load_hdf5(self.data_path+self.data_params['valid_desc'], start_pool, poolsize) 
            self._eval_sets={'methnames':methnames, 'tokens':tokens, 'descs':descs} # 'apiseqs':apiseqs,
            
        accs,mrrs,maps,ndcgs, sr1s, sr5s, sr10s = [], [], [], [], [], [], []
        ...
        for i in tqdm(range(data_len)):
            ...
            n_results = K          
            sims = model.predict([methnames,tokens, descs], batch_size=data_len).flatten() #apiseqs
            ...
            sr1s.append(SuccessRate_Avg(real,predict, 1)) 
            sr5s.append(SuccessRate_Avg(real,predict, 5)) 
            sr10s.append(SuccessRate_Avg(real,predict, 10))  
        acc, mrr, map_, ndcg, sr1, sr5, sr10 = np.mean(accs), np.mean(mrrs), np.mean(maps), np.mean(ndcgs), np.mean(sr1s), np.mean(sr5s), np.mean(sr10s)
        logger.info(f'ACC={acc}, MRR={mrr}, MAP={map_}, nDCG={ndcg}, SuccessRate_1={sr1}, SuccessRate_5={sr5}, SuccessRate_10={sr10}')        
        return accs,mrrs,maps,ndcgs, sr1s, sr5s, sr10s
    ...
    def repr_code(self, model):
        ...
        vecs= model.repr_code([methnames, tokens], batch_size=10000) #apiseqs
...
if __name__ == '__main__':
    ...
    if args.mode=='train':  
        ...
        if config['data_params']['run_test_in_batch']> 0:
            test_batch_size = 1000
            accs,mrrs,maps,ndcgs, sr1s, sr5s, sr10s = [], [], [], [], [], [], []
            for i in range(int(config['data_params']['size_testset']/test_batch_size)):
                start_batch = i*test_batch_size
                acc,mrr,map_,ndcg, sr1, sr5, sr10 = engine.valid(model, test_batch_size, 10, start_batch)
                accs.extend(acc)
                mrrs.extend(mrr)
                maps.extend(map_)
                ndcgs.extend(ndcg) 
                ...
                acc, mrr, map_, ndcg, sr1, sr5, sr10 = np.mean(accs), np.mean(mrrs), np.mean(maps), np.mean(ndcgs), np.mean(sr1s), np.mean(sr5s), np.mean(sr10s)
                logger.info(f'Final scores: ACC={acc}, MRR={mrr}, MAP={map_}, nDCG={ndcg}, SuccessRate_1={sr1}, SuccessRate_5={sr5}, SuccessRate_10={sr10}')
                scores = {'ACC': accs, 'MRR': mrrs, 'MAP': maps, 'nDCG': ndcgs, 'SuccessRate_1': sr1s, 'SuccessRate_5':sr5s, 'SuccessRate_10': sr10s}
                with open('dcs_scores.json', 'w') as f:
                    json.dump(scores, f)
            ...
            acc,mrr,map_,ndcg, sr1, sr5, sr10 = engine.valid(model, -1, 10)
            scores = {'ACC': acc, 'MRR': mrr, 'MAP': map, 'nDCG': ndcg, 'SuccessRate_1': sr1, 'SuccessRate_5':sr5, 'SuccessRate_10': sr10}
            with open('dcs_scores.json', 'w') as f:
                json.dump(scores, f)
