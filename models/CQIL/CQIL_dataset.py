...
class ValDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        name_lex = np.asarray(np.load(self.config['data_dir'] + self.dataset_type + '/' + str(index) + 'query_name.npz')['arr_0'])
        body_lex = np.asarray(np.load(
            self.config['data_dir'] + self.dataset_type + '/' + str(index) + 'query_body.npz')['arr_0'])
...
class TestDataset(data.Dataset):
    def __init__(self, config, dataset_type, dataset_file = None):
        self.config = config

        if not dataset_file:
            dataset_file = json.loads(open(config['data_dir'] + config['eval'], 'r').readline())
        self.data = dataset_file

        logger.info("loading data...")

        if dataset_type == 'recom':
            config['recom_qeury'] = {'query': input("Input query: ")}
            self.query_data = config['recom_qeury'] 
        else:
            
            self.query_data = json.loads(open(self.config['data_dir'] + config['eval2'], 'r').readline())
        ...
        if dataset_type == 'recom':
            ...
            for q in tqdm(self.query_data):
                self.query.append(q['query'].strip().split(' '))  
...
def get_queries(config):
    ...
    query_vocab = json.loads(open(config['data_dir'] + config['query_vocab_path'], "r").readline())
    query_data = json.loads(open(config['data_dir'] + config['eval2'], 'r').readline())
    name_lex, body_lex, query_list = [], [], []
    for index, q in enumerate(query_data):
        query_list.append(q['query'].strip().split(' '))
        name_lex.append(np.load(config['data_dir'] + 'eval' + '2' +'/' + str(index) + 'query_name.npz'))
        body_lex.append(np.load(
            config['data_dir'] + 'eval' + '2' +'/' + str(index) + 'query_body.npz'))   
    
    return [pad_seq(raw_query, max_query_len, query_vocab) for raw_query in query_list], name_lex, body_lex
