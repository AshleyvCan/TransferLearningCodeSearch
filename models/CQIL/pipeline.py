...
import fasttext
import numpy as np
from numpy import save, savez_compressed
from tqdm import tqdm

from CQIL_dataset import get_lex_match, pad_seq
...
class LexHelper():
    ...
    def generate_idf_matrix(self, dataset, lex_matrix_save_path):
        ...
        for begin in range(0, data_size, self.batch_size):
            ...
            if  begin + self.batch_size> len(self.query): batch_end = len(self.query)
            else: batch_end = begin + self.batch_size
            for index in tqdm(range(begin, batch_end)):
                raw_query = self.query[index]
                query_name, query_body = self.get_match_batch(raw_query, begin, self.name, self.body, batch_end)

                savez_compressed(lex_matrix_save_path + str(index) + 'query_name', query_name)
                savez_compressed(lex_matrix_save_path + str(index) + 'query_body', query_body)

    
    def get_match_batch(self, raw_query, batch_begin, raw_names, raw_bodies, batch_end = None):
        query_name_lex_matrices, query_body_lex_matrices = [], []
        if not batch_begin: batch_end =  batch_begin + self.batch_size
...
class LexHelper_recom():
    ...
    def run(self, config):
            self.generate_idf_matrix_recom(self.config['eval'], self.config['eval2'], self.config['data_dir'] + self.config['eval_lex2'], config)


    def generate_idf_matrix_recom(self, dataset, queries, lex_matrix_save_path, config):
        logger.info(f'{dataset}')
        self.data = json.loads(open(self.config['data_dir'] + dataset, 'r').readline())
      
        self.query_data = config['recom_qeury'] 
        ...
        self.query.append(self.query_data['query'].strip().split(' '))
        data_size = len(self.data)
        query_name, query_body = self.get_match_batch(self.query_data['query'].strip().split(' '), data_size, self.name, self.body)
        savez_compressed(lex_matrix_save_path + 'recom' + 'query_name', query_name)
        savez_compressed(lex_matrix_save_path + 'recom' + 'query_body', query_body)
...
class Pipeline():
    ...
    def create_train_temp_file(self):
        ...
        with open(self.train_temp_filepath, 'w',encoding="utf-8") as f:
            
            for index in tqdm(range(len(self.query))):

                f.write(' '.join(self.query[index]) + '\n')
                f.write(' '.join(self.name[index]) + '\n')
                body_i = list(filter(None, self.body[index]))
                f.write(' '.join(body_i).replace("\>", "")+ '\n' )
    ...
    def generate_SM_matrix(self, dataset, SM_matrix_save_path):
        self.data = json.loads(open(self.config['data_dir'] + dataset, 'r').readline())
        ...
        for i, code_snippet in enumerate(self.data):
            
            #print(len(raw_name))
            name = pad_seq(code_snippet['name'], self.config['max_name_len'], self.name_vocab)
            
            body = pad_seq(code_snippet['body'], self.config['max_body_len'], self.body_vocab)

            save(SM_matrix_save_path + str(i) + 'query_name', name)
            save(SM_matrix_save_path + str(i) + 'query_body', body)

    def run(self, config):

        if config['type'] == 'recom':
            lexhelper=LexHelper_recom(self.config)
            lexhelper.run(self.config)
        else:
            self.get_vocab()
